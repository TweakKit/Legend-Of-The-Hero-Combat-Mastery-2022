...
namespace Runtime.Gameplay.CollisionDetection
{
    public sealed class CollisionSystem : MonoSingleton<CollisionSystem>, IDisposable
    {
        ...
        protected override void Awake()
        {
            base.Awake();
            _currentBodyCount = 0;
            var stageMapWidth = MapManager.Instance.Width * MapManager.Instance.SlotSize;
            var stageMapHeight = MapManager.Instance.Height * MapManager.Instance.SlotSize;
            var stageMapCenter = MapManager.Instance.Center;
            InitQuadTree(new RectConfig(stageMapCenter, stageMapWidth, stageMapHeight), 10, 5);
        }
        ...
        public void InitQuadTree(RectConfig rect, int maxBodiesPerNode = 6, int maxLevel = 6)
        {
            ...
            _quadTrees[(int)CollisionSearchTargetType.Zombie] = new QuadTree(_bodyList, rect, maxBodiesPerNode, maxLevel);
        ...
        public void Dispose()
        {
            ...
            _collidedPairCache?.Dispose();
            _collidedPair?.Dispose();
            _refIdsQueue?.Dispose();
        }
        ...
        private void AddBodyToQuadTree(ICollisionBody collisionBody)
        {
            switch (collisionBody.CollisionBodyType)
            {
                ...
                case CollisionBodyType.Zombie:
                    _quadTrees[(int)CollisionSearchTargetType.Zombie]?.AddBody(collisionBody.RefId);
                    break;
        ...
        public void Step()
        {
            ...
            for (int i = 0; i <= _currentBodyCount; i++)
            {
                ...
                var collisionSearchTargetTypeLength = _bodyList[i].CollisionSearchTargetTypes.Length;
                for (int j = 0; j < collisionSearchTargetTypeLength; j++)
                {
                    if (_bodyList[i] == null)
                        break;
