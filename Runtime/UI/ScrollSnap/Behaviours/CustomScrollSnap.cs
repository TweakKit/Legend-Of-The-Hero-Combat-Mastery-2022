...
{
    [AddComponentMenu("UI/Custom Scroll Snap")]
    [RequireComponent(typeof(ScrollRect))]
    public class CustomScrollSnap : MonoBehaviour, IDragHandler, IBeginDragHandler, IEndDragHandler, IPointerDownHandler, IPointerUpHandler
    {
        #region Membres

        [Header("=== MOVEMENT AND LAYOUT SETTINGS ===")]
        [SerializeField]
        private MovementType movementType = MovementType.Fixed;
        [SerializeField]
        private MovementAxis movementAxis = MovementAxis.Horizontal;
        [SerializeField]
        private bool useAutomaticLayout = true;
        [SerializeField]
        private float automaticLayoutSpacing = 0.25f;
        [SerializeField]
        private SizeControl sizeControl = SizeControl.Fit;
        [SerializeField]
        private Vector2 size = new Vector2(400, 250);
        [SerializeField]
        private Margins automaticLayoutMargins = new Margins(0);
        [SerializeField]
        private bool useInfiniteScrolling = false;
        [SerializeField]
        private float infiniteScrollingSpacing = 0.25f;
        [SerializeField]
        private bool useOcclusionCulling = false;
        [SerializeField]
        private int startingPanel = 0;

        [Header("=== NAVIGATION SETTINGS ===")]
        [SerializeField]
        private bool useSwipeGestures = true;
        [SerializeField]
        private float minimumSwipeSpeed = 0f;
        [SerializeField]
        ...
        [SerializeField]
        private bool useToggleNavigation = true;

        [Header("=== SNAP SETTINGS ===")]
        [SerializeField]
        private SnapTarget snapTarget = SnapTarget.Next;
        [SerializeField]
        private float snapSpeed = 10f;
        [SerializeField]
        private float thresholdSpeedToSnap = -1f;
        [SerializeField]
        private bool useHardSnapping = true;
        [SerializeField]
        ...
        private Action<int> OnPanelSelectingCallback { get; set; }
        private Action<int> OnPanelSelectedCallback { get; set; }
        private Action<int, int> OnPanelCenteringCallback { get; set; }
        private Action<int, int> OnPanelCenteredCallback { get; set; }
        ...
        {
            if (isDragging)
                OnPanelSelectingCallback?.Invoke(GetNearestPanel());
        }
        ...
        public void AddOnPanelSelectingCallbackListener(Action<int> onPanelSelectingCallbackListener)
            => OnPanelSelectingCallback += onPanelSelectingCallbackListener;

        public void AddOnPanelSelectedCallbackListener(Action<int> onPanelSelectedCallbackListener)
            => OnPanelSelectedCallback += onPanelSelectedCallbackListener;

        public void AddOnPanelCenteringCallbackListener(Action<int, int> onPanelCenteringCallbackListener)
            => OnPanelCenteringCallback += onPanelCenteringCallbackListener;

        public void AddOnPanelCenteredCallbackListener(Action<int, int> onPanelCenteredCallbackListener)
            => OnPanelCenteredCallback += onPanelCenteredCallbackListener;
        ...
        private void SnapToPanel()
        {
            ...
            if (SelectedPanel != CenteredPanel)
            {
                ...
                {
                    OnPanelCenteredCallback?.Invoke(CenteredPanel, SelectedPanel);
                    SelectedPanel = CenteredPanel;
                }
            }
            else
            {
                OnPanelCenteringCallback?.Invoke(CenteredPanel, SelectedPanel);
            }
        }
        ...
        public void GoToPanel(int panelNumber)
        {
            ...
            isSelected = true;
            OnPanelSelectedCallback?.Invoke(panelNumber);
